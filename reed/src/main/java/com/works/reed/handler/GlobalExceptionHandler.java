...
//                .validationErrors(exp.getViolations())
    ...
    public ExceptionResponse handle(MethodArgumentNotValidException exp) {
        return ExceptionResponse
                .builder()
                .errorMsg("validation failed for argument")
                .errorSource("")
                .validationErrors(exp.getBindingResult().getFieldErrors().stream().map(e -> e.getDefaultMessage()).collect(Collectors.toList()))
                .build();
    }
